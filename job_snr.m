...
for k = 1:length(SNRspec)
    ...
    if (sig>ISI) & -10*log10((sig-ISI)/eyeamp) < ISIpen
        ISIpen = -10*log10((sig-ISI)/eyeamp); 
        c_fs = c;
    end
    end
    end
...
for k = 1:length(SNRspec)
    ...
    % sample the channel response
    fpulsesampled = fpulse(phase:OSR:end); % sample the pulse response
...
semilogy(SNRspec(1:length(BER_br)),BER_br,'b<-',SNRspec(1:length(BER_fs)),BER_fs,'kx-',SNRspec(1:length(BER_dfe)),BER_dfe,'ro-',SNRspec(1:length(BER_dfefs)),BER_dfefs,'m^-',SNRspec(1:length(BER_mlse)),BER_mlse,'gs-','linewidth',2); set(gca,'linewidth',3,'fontsize',14,'fontname','times'); ylim([1e-20 1]);
